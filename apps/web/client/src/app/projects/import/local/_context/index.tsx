...
async function seedLocalProjectFiles(
    projectId: string,
    branchId: string,
    files: ProcessedFile[],
): Promise<void> {
    const codeFileSystem = new CodeFileSystem(projectId, branchId);
    await codeFileSystem.initialize();
    await codeFileSystem.writeFiles(
        files.map((file) => ({
            path: file.path,
            content:
    ...
    await codeFileSystem.rebuildIndex();
}
...
export const ProjectCreationProvider = ({ children, totalSteps }: ProjectCreationProviderProps) => {
    ...
    const finalizeProject = async () => {
        try {
            ...
            const port = detectPortFromPackageJson(packageJsonFile);

            if (LOCAL_MODE_ENABLED) {
                // In local mode: skip sandbox creation and file upload.
                // The project files are already on the local machine.
                const sandboxId = `local-${user.id.slice(0, 8)}`;
                const sandboxUrl = getLocalPreviewUrl(port);
                const project = await createProject({
                    ...
                    sandboxId,
                    sandboxUrl,
                    folderPath: projectData.folderPath ?? null,
                    userId: user.id,
                });
                if (!project) {
