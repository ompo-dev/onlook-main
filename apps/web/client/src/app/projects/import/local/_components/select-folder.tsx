...
export const NewSelectFolder = () => {
    ...
    const deriveAbsoluteFolderPath = (file: File | undefined): string | null => {
        const localFile = file as File & {
            path?: string;
            webkitRelativePath?: string;
        };
        const absoluteFilePath = localFile.path;
        const relativePath = localFile.webkitRelativePath;
        ...
        const innerRelativePath = segments.slice(1).join('/');
        const normalizedAbsoluteFilePath = absoluteFilePath.replace(/\\/g, '/');

        if (innerRelativePath && normalizedAbsoluteFilePath.endsWith(innerRelativePath)) {
            const folderPath = normalizedAbsoluteFilePath
                .slice(0, normalizedAbsoluteFilePath.length - innerRelativePath.length)
                .replace(/\/$/, '');

            return absoluteFilePath.includes('\\')
    ...
    const processProjectFiles = async (fileList: FileList | File[]) => {
        ...
        try {
            ...
            const absoluteFolderPath = deriveAbsoluteFolderPath(files[0]);
