...
import { resolveLocalPreviewFrameUrl } from '@/utils/local-mode/preview-url';
...
export class FramesManager {
    ...
    registerView(frame: Frame, view: IFrameView) {
        ...
        let framePathname = '/';
        try { framePathname = new URL(view.src).pathname; } catch { /* empty src */ }
    ...
    async navigateToPath(frameId: string, path: string, addToHistory = true): Promise<void> {
        ...
        try {
            const currentUrl = resolveLocalPreviewFrameUrl(frameData.frame.url) ?? frameData.view.src;
