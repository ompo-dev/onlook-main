...
const isRouteGroupDirectory = (segment: string): boolean => {
...
const getPublicRouteSegment = (segment: string): string => {
    return isRouteGroupDirectory(segment) ? '' : segment;
};

const routeSegmentMatchesDirectory = (directoryName: string, routeSegment: string): boolean => {
    return directoryName === routeSegment || isDynamicRouteSegment(directoryName);
};
...
export const scanAppDirectory = async (
    ...
    if (pageFile) {
        ...
        const childPromises = childDirectories.map((entry) => {
            ...
            const relativePath = joinPath(parentPath, getPublicRouteSegment(entry.name));
        ...
        const childPromises = childDirectories.map(async (entry) => {
            ...
            if (children.length > 0) {
                ...
                if (isRouteGroupDirectory(currentDirName)) {
                    return children;
                }
...
const resolveAppDirectoryPathForRoute = async (
    sandboxManager: SandboxManager,
    basePath: string,
    routePath: string,
): Promise<string | null> => {
    ...
    const findMatchingDirectory = async (
        currentDir: string,
        ...
        try {
            entries = await sandboxManager.readDir(currentDir);
        } catch {
            return null;
        }

        const { pageFile } = getPageAndLayoutFiles(entries);
        if (remainingSegments.length === 0 && pageFile) {
            return currentDir;
        }
        ...
        for (const entry of childDirectories) {
            if (!isRouteGroupDirectory(entry.name)) {
                continue;
            }

            const groupedResult = await findMatchingDirectory(
        ...
        for (const entry of childDirectories) {
            ...
            if (!routeSegmentMatchesDirectory(entry.name, nextSegment)) {
                continue;
            }

            const matchedResult = await findMatchingDirectory(
                joinPath(currentDir, entry.name),
                restSegments,
            );
            if (matchedResult) {
    ...
    return findMatchingDirectory(basePath, segments);
};
...
export const deletePageInSandbox = async (
    ...
    try {
        ...
        const resolvedPath =
            (await resolveAppDirectoryPathForRoute(
                sandboxManager,
                routerConfig.basePath,
                normalizedPath,
            )) ?? joinPath(routerConfig.basePath, normalizedPath);
    ...
    try {
        ...
        const oldFullPath =
            (await resolveAppDirectoryPathForRoute(
                ...
                routerConfig.basePath,
                normalizedOldPath,
            )) ?? joinPath(routerConfig.basePath, normalizedOldPath);
...
export const duplicatePageInSandbox = async (
    ...
    try {
        ...
        const sourceFull =
            ...
            )) ?? joinPath(routerConfig.basePath, normalizedSourcePath);
...
export const updatePageMetadataInSandbox = async (
    ...
    const fullPath =
        (await resolveAppDirectoryPathForRoute(
            ...
            routerConfig.basePath,
            normalizedPagePath,
        )) ?? joinPath(routerConfig.basePath, normalizedPagePath);
    const pageFilePath = joinPath(fullPath, 'page.tsx');
