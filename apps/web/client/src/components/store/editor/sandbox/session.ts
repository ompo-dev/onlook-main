...
import { CodeProvider, createCodeProviderClient, NodeFsProvider, type Provider } from '@onlook/code-provider';
...
export class SessionManager {
    ...
    async start(sandboxId: string, userId?: string): Promise<void> {
        ...
        const attemptConnection = async () => {
            ...
            if (LOCAL_MODE_ENABLED) {
                provider = new NodeFsProvider({
                    folderPath: this.branch.sandbox.folderPath ?? null,
            ...
            runInAction(() => { this.provider = provider; });
        ...
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                runInAction(() => { this.provider = null; });
    ...
    async createTerminalSessions(provider: Provider) {
        ...
        const task = new CLISessionImpl(
            LOCAL_MODE_ENABLED ? 'local preview' : 'server',
            CLISessionType.TASK,
            provider,
            this.errorManager,
        );
        ...
        if (LOCAL_MODE_ENABLED) {
            try {
                ...
                console.error('Failed to initialize local preview task:', error);
            }
            return;
        }
