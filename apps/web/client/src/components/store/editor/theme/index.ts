...
export class ThemeManager {
    ...
    private detectedFontFamilies: string[] = [];
    ...
    async scanConfig() {
        try {
            ...
            if (!configResult) {
                await this.scanProjectStylesFallback();
                return;
            }
            ...
            this.detectedFontFamilies = await this.detectFontFamiliesFromProject();
        } catch (error) {
            console.error('Error loading colors:', error);
        }
    }

    private async scanProjectStylesFallback() {
        this.configPath = null;
        this.cssPath = null;
        this.defaultColors = {};
        this.brandColors = {};

        const styleEntries = await this.editorEngine.activeSandbox.listAllFiles();
        const candidateFiles = styleEntries.filter((entry) => {
            if (entry.type !== 'file') {
                return false;
            }
            ...
            return (
                /(global|globals|style|styles)\.(ts|tsx|css|scss|sass|less)$/.test(normalizedPath) ||
                /\/app\/(layout|fonts)\.(ts|tsx)$/.test(normalizedPath)
            );
        });

        const detectedColors: Record<string, TailwindColor[]> = {};

        for (const entry of candidateFiles) {
            const content = await this.editorEngine.activeSandbox.readFile(entry.path);
            if (typeof content !== 'string') {
                continue;
            }

            const cssVariableMatches = content.matchAll(/--([\w-]+)\s*:\s*([^;]+);/g);
            for (const match of cssVariableMatches) {
                ...
                const normalizedColor = Color.from(variableValue)?.toHex();
                if (!normalizedColor) {
                    continue;
                }

                const [groupName, ...shadeParts] = variableName.split('-');
                if (!groupName) {
                    continue;
                }

                const shadeName = shadeParts.length > 0 ? shadeParts.join('-') : DEFAULT_COLOR_NAME;
                const existingGroup = detectedColors[groupName] ?? [];
                if (existingGroup.some((color) => color.originalKey === variableName)) {
                    continue;
                }

                existingGroup.push({
                    name: shadeName,
                    originalKey: variableName,
                    lightColor: normalizedColor,
                    darkColor: normalizedColor,
                });

                detectedColors[groupName] = existingGroup;
            }
        }

        Object.values(detectedColors).forEach((group) => {
            group.sort((left, right) => {
                if (left.name === DEFAULT_COLOR_NAME) {
                    return -1;
                }
                if (right.name === DEFAULT_COLOR_NAME) {
                    return 1;
                }
        ...
        this.brandColors = detectedColors;
        this.detectedFontFamilies = await this.detectFontFamiliesFromProject(candidateFiles);
    }

    private normalizeFontFamilyCandidate(value: string): string | null {
        const firstCandidate = value
            .replace(/['"]/g, '')
            .split(',')[0]
            ?.trim();

        if (!firstCandidate || firstCandidate.includes('var(--')) {
            return null;
        }

        const normalized = firstCandidate.toLowerCase();
        if (
            ['sans-serif', 'serif', 'monospace', 'system-ui', 'inherit', 'initial'].includes(
                normalized,
            )
        ) {
            return null;
    ...
    private async detectFontFamiliesFromProject(
        candidateFiles?: Array<{ path: string; type: 'file' | 'directory' }>,
    ): Promise<string[]> {
        const entries =
            candidateFiles ??
            (await this.editorEngine.activeSandbox.listAllFiles()).filter((entry) => entry.type === 'file');
        ...
        for (const entry of entries) {
            ...
            const fontFamilyMatches = content.matchAll(/font-family\s*:\s*([^;]+);/gi);
            for (const match of fontFamilyMatches) {
                const family = this.normalizeFontFamilyCandidate(match[1] ?? '');
                if (family) {
                    detectedFamilies.add(family);
                }
            }

            const cssFontVariableMatches = content.matchAll(/--[\w-]*font[\w-]*\s*:\s*([^;]+);/gi);
            for (const match of cssFontVariableMatches) {
                const family = this.normalizeFontFamilyCandidate(match[1] ?? '');
                if (family) {
                    detectedFamilies.add(family);
                }
            ...
            const nextFontImportMatches = content.matchAll(
                /import\s*\{\s*([^}]+)\s*\}\s*from\s*['"]next\/font\/google['"]/g,
            );
            for (const match of nextFontImportMatches) {
                const importedFonts = (match[1] ?? '')
                    .split(',')
                    .map((value) => value.trim())
                    .filter(Boolean);

                for (const importedFont of importedFonts) {
                    detectedFamilies.add(importedFont);
                }
            }
        }

        return Array.from(detectedFamilies).sort((left, right) => left.localeCompare(right));
    }
    ...
    get projectFontFamilies() {
        return this.detectedFontFamilies;
    }
