...
export function useKeyboardShortcuts({
    ...
    useEffect(() => {
        const handler = (e: KeyboardEvent) => {
            ...
            if (key === 'ArrowUp' || key === 'ArrowDown') {
                ...
                if (nextIdx >= 0 && nextIdx < visible.length && nextId !== undefined) {
                ...
                if (!store.expandedNodes[store.selectedNodeId]) {
                    ...
                    if (firstChild) {
                        handleSelectNode(firstChild.id);
                    }
                }
                ...
                if (store.expandedNodes[store.selectedNodeId]) {
                    ...
                    const parentNode = path && path.length >= 2 ? path[path.length - 2] : null;
                    if (parentNode) {
    ...
    useEffect(() => {
        const handler = (e: PointerEvent) => {
            ...
            if (
                target.closest?.(
                    'css-studio-panel,#css-studio-root,[data-cs-root],[data-cs-floating],[data-cs-panel],[data-cs-toolbar]',
                )
            ) {
                return;
            }
