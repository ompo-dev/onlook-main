...
const FILTER_CONFIG_MAP = new Map<string, FilterConfig>(FILTER_CONFIGS.map((c) => [c.name, c]));

export function parseFilter(value: string): FilterEntry[] {
...
export function serializeFilter(filters: FilterEntry[]): string {
  if (filters.length === 0) return '';
  return filters.map((f) => `${f.name}(${f.args})`).join(' ');
}

export function getFilterConfig(name: string): FilterConfig | undefined {
