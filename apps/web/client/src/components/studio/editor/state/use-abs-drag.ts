...
export function useAbsDrag({
    bridge,
    onPositionChange,
}: {
    bridge: PageBridge;
    onPositionChange: (id: number, changes: { property: string; oldValue: string; newValue: string }[]) => void;
}) {
    ...
    const getAbsInfo = useCallback(
        (nodeId: number) => bridge.getAbsolutePositionInfo(nodeId),
        [bridge],
    );
