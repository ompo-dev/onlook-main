...
export const projectRouter = createTRPCRouter({
    hasAccess: protectedProcedure
        ...
        .query(async ({ ctx, input }) => {
            if (LOCAL_MODE_ENABLED) {
                return localGetProject(input.projectId) !== null;
            }
            const user = ctx.user;
    ...
    list: protectedProcedure
        ...
        .query(async ({ ctx, input }) => {
            if (LOCAL_MODE_ENABLED) {
                let projects = localListProjects(ctx.user.id).map(toModelProject);
                if (input?.excludeProjectId) {
                    projects = projects.filter((p) => p.id !== input.excludeProjectId);
                }
                if (input?.limit) projects = projects.slice(0, input.limit);
    ...
    get: protectedProcedure
        ...
        .query(async ({ ctx, input }) => {
            if (LOCAL_MODE_ENABLED) {
                const p = localGetProject(input.projectId);
                return p ? toModelProject(p) : null;
            }
            const project = await ctx.db.query.projects.findFirst({
                where: eq(projects.id, input.projectId),
            });
    ...
    getProjectWithCanvas: protectedProcedure
        ...
        .query(async ({ ctx, input }) => {
            if (LOCAL_MODE_ENABLED) {
                const result = localGetProjectWithCanvas(input.projectId, ctx.user.id);
                if (!result) return null;
                return {
                    project: toModelProject(result.project),
                    userCanvas: {
                        id: result.canvas.id,
                    ...
                    frames: result.frames.map((f) => ({
                        ...
                        canvasId: f.canvasId,
                        url: f.url,
                        position: { x: parseInt(f.x), y: parseInt(f.y) },
                        dimension: { width: parseInt(f.width), height: parseInt(f.height) },
                    })),
                };
            }
            const project = await ctx.db.query.projects.findFirst({
    ...
    create: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            if (LOCAL_MODE_ENABLED) {
                const stored = localCreateProject({
                    name: input.project.name ?? 'New Project',
                ...
                return { id: stored.id, name: stored.name, defaultBranchId: stored.defaultBranchId };
            }
    ...
    update: protectedProcedure.input(projectUpdateSchema).mutation(async ({ ctx, input }) => {
        if (LOCAL_MODE_ENABLED) {
            const updated = localUpdateProject(input.id, {
                name: input.name ?? undefined,
                description: input.description ?? undefined,
                tags: input.tags ?? undefined,
            });
            return updated ? toModelProject(updated) : null;
        }
