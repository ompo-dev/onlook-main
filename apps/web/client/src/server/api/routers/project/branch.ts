...
import { localGetBranches, localGetProject, toModelBranch } from '@/utils/local-mode/local-store';
import { getLocalPreviewUrl } from '@/utils/local-mode/preview-url';
    ...
    getByProjectId: protectedProcedure
        ...
        .query(async ({ ctx, input }) => {
            if (LOCAL_MODE_ENABLED) {
                const localBranches = localGetBranches(input.projectId, input.onlyDefault);
                if (!localBranches.length) {
                    throw new TRPCError({ code: 'NOT_FOUND', message: 'Branches not found' });
                }
                const project = localGetProject(input.projectId);
                return localBranches.map(b => toModelBranch(b, project?.folderPath));
            }
    ...
    fork: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            try {
                ...
                const Provider = LOCAL_MODE_ENABLED
                    ? await getStaticCodeProvider(CodeProvider.NodeFs)
                    : await getStaticCodeProvider(CodeProvider.CodeSandbox);

                const forkedSandbox = await Provider.createProject({
                    source: 'template',
                    id: LOCAL_MODE_ENABLED ? uuidv4() : sourceBranch.sandboxId,
                    title: branchName,
                    tags: ['fork'],
                });
                ...
                const previewUrl = LOCAL_MODE_ENABLED
                    ? getLocalPreviewUrl(port)
                    : getSandboxPreviewUrl(sandboxId, port);
        ...
        .mutation(async ({ ctx, input }) => {
            try {
                return await ctx.db.transaction(async (tx) => {
                    ...
                    const BlankProvider = LOCAL_MODE_ENABLED
                        ? await getStaticCodeProvider(CodeProvider.NodeFs)
                        : await getStaticCodeProvider(CodeProvider.CodeSandbox);

                    const blankSandbox = await BlankProvider.createProject({
                        source: 'template',
                        id: LOCAL_MODE_ENABLED ? uuidv4() : SandboxTemplates[Templates.EMPTY_NEXTJS].id,
                        title: branchName,
                        tags: ['blank'],
                    });
